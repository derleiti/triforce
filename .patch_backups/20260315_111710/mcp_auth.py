...
async def require_mcp_auth(request: Request) -> str:
    ...
    if forwarded_port == "9100":
        ...
        # 2. IP-BYPASS: Echter interner/Docker Traffic (kein X-Forwarded-Port: 9100)
        _PRIVATE_PREFIXES = (
        ...
        # 3. Kein Port, keine private IP → auch bypass (public/direkt)
        logger.debug(f"AUTH_OK | IP: {client_ip} | X-Fwd-Port: none | Method: port_bypass")
        return "internal"
