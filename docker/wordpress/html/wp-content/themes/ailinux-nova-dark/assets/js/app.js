const AilinuxNovaApp = (() => {
  ...
  const disableSwupOnBbpress = () => {
    document.querySelectorAll('a[href*="/forum"], a[href*="/topics"], a.bbp-topic-permalink')
  ...
  const setupAiDiscuss = () => {
    ...
    const sendButton = document.getElementById('ai-discuss-send');
    const input = document.getElementById('ai-discuss-input');
    const output = document.getElementById('ai-discuss-chat');
    ...
    const getApiConfig = () => {
      ...
      const defaultModel = (window.NOVA_API && window.NOVA_API.DEFAULT_MODEL) || 'gpt-oss:120b-cloud';
      ...
      const safeBase = (() => {
        try {
          ...
          if (isLocalHost && url.port === '9000' && url.protocol === 'https:') {
            url.protocol = 'http:';
      ...
      return {
        chatUrl: `${safeBase}${chatEndpoint.startsWith('/') ? chatEndpoint : `/${chatEndpoint}`}`,
        modelsUrl: `${safeBase}${modelsEndpoint.startsWith('/') ? modelsEndpoint : `/${modelsEndpoint}`}`,
        healthUrl: `${safeBase}${healthEndpoint.startsWith('/') ? healthEndpoint : `/${healthEndpoint}`}`,
    ...
    // Fallback model list with all available models + free-tier fallbacks
    const FALLBACK_MODELS = [
      { id: 'gpt-oss:120b-cloud', name: '⭐ GPT-OSS 120B (Ollama Cloud)' },
      { id: 'gpt-oss:20b-cloud', name: 'GPT-OSS 20B Fast (Ollama Cloud)' },
      { id: 'kimi-k2-thinking:cloud', name: '🧠 Kimi K2 Thinking 1T (Reasoning)' },
      { id: 'qwen3-coder:480b-cloud', name: '💻 Qwen3 Coder 480B (Code)' },
      { id: 'deepseek-v3.2:cloud', name: '🔬 DeepSeek V3.2 671B (Research)' },
      { id: 'glm-5:cloud', name: 'GLM-5 (General)' },
      { id: 'glm-4.7:cloud', name: 'GLM-4.7 (General)' },
      { id: 'glm-4.7-flash:latest', name: '⚡ GLM-4.7 Flash 30B (Local)' },
      { id: 'qwen3:8b', name: 'Qwen3 8B (Local Fast)' },
      { id: 'gemini-2.5-flash', name: '🌟 Gemini 2.5 Flash' },
      { id: 'minimax-m2.5:cloud', name: 'MiniMax M2.5 (Cloud)' },
      { id: 'kimi-k2.5:cloud', name: 'Kimi K2.5 (Cloud)' },
      { id: 'devstral-2:123b-cloud', name: '💻 Devstral 2 123B (Code)' },
      { id: 'cogito-2.1:671b-cloud', name: '🧠 Cogito 2.1 671B (Reasoning)' },
    ];

    const populateFallbackModels = () => {
      if (!modelSelect || !modelSelectWrapper) return;
      ...
      console.log('[DiscussAI] Using fallback model list:', FALLBACK_MODELS.length, 'models');
    };
    ...
    // Curated model groups for the Discuss panel
    const CURATED = {
      'Empfohlen': [
        'ollama/gpt-oss:120b-cloud', 'ollama/kimi-k2-thinking:cloud',
        'ollama/deepseek-v3.2:cloud', 'ollama/qwen3-coder:480b-cloud',
        'gemini/gemini-2.5-flash',
      ],
      'Code': [
        'ollama/devstral-2:123b-cloud', 'ollama/devstral-small-2:24b-cloud',
        'ollama/qwen3-coder-next:cloud', 'mistral/codestral-latest',
      ],
      'Reasoning': [
        'ollama/cogito-2.1:671b-cloud', 'ollama/glm-5:cloud',
        'ollama/kimi-k2.5:cloud', 'ollama/qwen3.5:cloud',
      ],
      'Schnell': [
        'ollama/gpt-oss:20b-cloud', 'ollama/glm-4.7-flash:latest',
        'ollama/qwen3:8b', 'ollama/ministral-3:8b-cloud',
      ],
      'Cloud': [
        'ollama/minimax-m2.5:cloud', 'ollama/glm-4.7:cloud',
        'ollama/gemini-3-flash-preview:cloud', 'ollama/qwen3-vl:235b-cloud',
      ],
    };
    const CURATED_IDS = new Set(Object.values(CURATED).flat());

    const populateGrouped = (allModels) => {
      if (!modelSelect || !modelSelectWrapper) return;
      modelSelect.innerHTML = '';
      const available = new Set(allModels.map(m => m.id || m.name || m));
      let count = 0;
      for (const [group, ids] of Object.entries(CURATED)) {
        const matched = ids.filter(id => available.has(id));
        ...
        matched.forEach(id => {
          ...
          o.value = id;
          o.textContent = id.replace('ollama/','').replace(':cloud','').replace(':latest','');
          if (id === config.defaultModel) o.selected = true;
          og.appendChild(o);
          count++;
        });
      ...
      modelSelectWrapper.style.display = 'block';
      console.log('[DiscussAI] Curated', count, 'models from', allModels.length, 'total');
    };
    ...
    const loadModels = async () => {
      try {
        ...
        if (models.length > 0 && modelSelect && modelSelectWrapper) {
          populateGrouped(models);
        } else {
          populateFallbackModels();
        }
      } catch (error) {
        console.warn('[DiscussAI] Models fetch failed:', error.message);
        populateFallbackModels();
      }
    };
    ...
    const discuss = async () => {
      ...
      if (!config.chatUrl.startsWith('https://')) {
        console.warn('[DiscussAI] API is not HTTPS:', config.chatUrl);
      }
...
document.addEventListener('DOMContentLoaded', () => { if (typeof NOVA_API !== 'undefined' && NOVA_API.DISABLED) { console.log('[DiscussAI] Disabled - using Nova AI Plugin'); return; }
  AilinuxNovaApp.init();
  AilinuxNovaTransitions.initSwup();
});


/* ── Account Dropdown (JWT-based) ───────────────────────────────────────── */
(function initAccountNav() {
  const LS = { token:'ailinux_token', email:'ailinux_email', name:'ailinux_display_name', tier:'ailinux_tier' };
  const ls = k => { try { return localStorage.getItem(k); } catch(e) { return null; } };
  const lsRm = k => { try { localStorage.removeItem(k); } catch(e) {} };

  const token = ls(LS.token);
  const email = ls(LS.email);
  document.getElementById('account-nav')?.classList.add('js-ready');
  if (!token || !email) {
    document.getElementById('account-nav')?.classList.add('js-ready');
    return; // not logged in — show Login link
  }

  const rawName = ls(LS.name) || email.split('@')[0] || 'Account';
  ...
  const list      = dropdown?.querySelector('.account-dropdown__list');
  ...
  if (loginBtn && dropdown) {
    ...
    document.addEventListener('keydown', e => {
      if (e.key === 'Escape') { list?.classList.remove('is-open'); trigger?.setAttribute('aria-expanded','false'); }
    });

    // Show WP admin link if admin tier
    const tier = ls(LS.tier) || '';
    if (tier === 'admin' || tier === 'enterprise' || email.endsWith('@ailinux.me')) {
      document.getElementById('account-admin-link')?.classList.remove('hidden');
    }
  }
  ...
  // ── Logout ──
  const doLogout = () => {
    Object.values(LS).forEach(lsRm);
    ['ailinux_client_id','ailinux_wp_last_sync','ailinux_wp_can_admin'].forEach(lsRm);
    try { document.cookie = 'ailinux_token=;domain=.ailinux.me;path=/;max-age=0'; } catch(e) {}
    // WP-Session via wp_logout_url() invalidieren (hat Nonce)
    // AILINUX_AUTH.wpLogoutUrl wird von functions.php injiziert
    var wpLogout = (typeof AILINUX_AUTH !== 'undefined' && AILINUX_AUTH.wpLogoutUrl)
      ? AILINUX_AUTH.wpLogoutUrl
      : 'https://login.ailinux.me?action=logout';
    window.location.href = wpLogout;
  };
  document.getElementById('account-logout')?.addEventListener('click', e => { e.preventDefault(); doLogout(); });
  mobileLogout?.addEventListener('click', e => { e.preventDefault(); doLogout(); });
})();
