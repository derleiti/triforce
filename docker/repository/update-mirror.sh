...
# Entferne verwaiste apt-mirror Lock-Datei (falls Container vorher abgestürzt)
docker compose -f "$COMPOSE_FILE" exec -T "$SERVICE" rm -f /var/spool/apt-mirror/var/apt-mirror.lock 2>/dev/null || true
