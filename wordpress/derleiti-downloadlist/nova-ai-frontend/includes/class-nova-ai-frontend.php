...
class Frontend
{
    ...
    public static function init(): void
    {
        ...
        add_filter('the_content', [self::class, 'append_discussion_button'], 20);
    }
    ...
    public static function register_settings(): void
    {
        ...
        register_setting(self::OPTION_GROUP, self::OPTION_CRAWLER_FORUM, [
            'type' => 'integer',
            'sanitize_callback' => 'absint',
            'default' => 0,
        ]);
        ...
        add_settings_field(
            self::OPTION_CRAWLER_FORUM,
            __('bbPress forum ID', 'nova-ai-frontend'),
            [self::class, 'render_crawler_forum_field'],
            self::OPTION_PAGE,
            'nova_ai_crawler'
        );
    ...
    public static function render_crawler_forum_field(): void
    {
        $value = (int) get_option(self::OPTION_CRAWLER_FORUM, 0);
        printf(
            '<input type="number" class="small-text" name="%1$s" id="%1$s" value="%2$d" min="0" />' .
            '<p class="description">%3$s</p>',
            esc_attr(self::OPTION_CRAWLER_FORUM),
            $value,
            esc_html__('bbPress Forum-ID für neue Themen (0 = deaktiviert).', 'nova-ai-frontend')
        );
    }
    ...
    private static function process_crawler_publications(): void
    {
        ...
        $forumId = (int) get_option(self::OPTION_CRAWLER_FORUM, 0);
        ...
        foreach ($results as $result) {
            ...
            $topicId = null;
            if ($forumId > 0 && function_exists('bbp_insert_topic')) {
                $topicId = self::create_forum_topic($result, $postId, $forumId, $authorId);
            }

            self::notify_backend_publication($result['id'], $postId, $topicId);
        }
    }
    ...
    private static function create_forum_topic(array $result, int $postId, int $forumId, int $authorId): ?int
    {
        if (!function_exists('bbp_insert_topic')) {
            return null;
        }

        $topicContent = self::build_forum_content($result, get_permalink($postId));
        $topicArgs = [
            'post_parent' => $forumId,
            'post_author' => $authorId,
            'post_title' => self::prepare_title($result),
            'post_content' => $topicContent,
            'post_status' => function_exists('bbp_get_public_status_id') ? bbp_get_public_status_id() : 'publish',
        ];

        $topicId = bbp_insert_topic($topicArgs);
        if (is_wp_error($topicId)) {
            error_log('[Nova AI] Failed to create bbPress topic: ' . $topicId->get_error_message());
            return null;
        }
    ...
    private static function notify_backend_publication(string $resultId, ?int $postId, ?int $topicId): void
    {
    ...
    private static function build_forum_content(array $result, string $postPermalink): string
    {
        $sections = [];
        if (!empty($result['summary'])) {
            $sections[] = self::format_summary($result['summary']);
        }
        $sections[] = sprintf(
            '<p>%1$s <a href="%2$s">%3$s</a></p>',
            esc_html__('Discuss the full article on the site:', 'nova-ai-frontend'),
            esc_url($postPermalink),
            esc_html($postPermalink)
        );
        if (!empty($result['url'])) {
            $sections[] = sprintf(
                '<p>%1$s <a href="%2$s">%3$s</a></p>',
                esc_html__('Source:', 'nova-ai-frontend'),
                esc_url($result['url']),
                esc_html($result['url'])
            );
        }
