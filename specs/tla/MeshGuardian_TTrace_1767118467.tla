---- MODULE MeshGuardian_TTrace_1767118467 ----
...
_inv ==
    ~(
        ...
        /\
        nodeState = ([hetzner |-> "crashed", backup |-> "crashed"])
        /\
        guardianActive = (TRUE)
        /\
        failureCount = ([hetzner |-> 0, backup |-> 0])
        /\
        lastHealthCheck = ("-")
    )
----
...
\* to `JsonSerialize`. For example, a sub-sequence of _TETrace.
    \* ASSUME
    \*     LET J == INSTANCE Json
    \*         IN J!JsonSerialize("MeshGuardian_TTrace_1767118467.json", _TETrace)
...
\*
\*trace == IODeserialize("MeshGuardian_TTrace_1767118467.bin", TRUE)
\*
\*=============================================================================
\*
...
trace == 
    ...
    ([restartCount |-> [hetzner |-> 0, backup |-> 0],nodeState |-> [hetzner |-> "crashed", backup |-> "healthy"],guardianActive |-> TRUE,failureCount |-> [hetzner |-> 0, backup |-> 0],lastHealthCheck |-> "-"]),
    ([restartCount |-> [hetzner |-> 0, backup |-> 0],nodeState |-> [hetzner |-> "crashed", backup |-> "crashed"],guardianActive |-> TRUE,failureCount |-> [hetzner |-> 0, backup |-> 0],lastHealthCheck |-> "-"])
    >>
----
...
---- CONFIG MeshGuardian_TTrace_1767118467 ----
CONSTANTS
...
ALIAS
    _expression
=============================================================================
\* Generated on Tue Dec 30 19:14:28 CET 2025