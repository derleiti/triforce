...
async def handle_tools_list(params: Dict[str, Any], request: Optional[Request] = None) -> Dict[str, Any]:
    ...
    tools = get_unified_tools(extra_tools=_EXTRA_TOOL_SCHEMAS)
...
async def handle_tools_call(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if entry:
        ...
        if policy.get("decision") != "allow":
            ...
            fallback_eligible = canonical_name in {"code_edit", "code_patch"}
...
async def _execute_mcp_write_fallback(fallback_id: str) -> Dict[str, Any]:
    ...
    if tool_name not in {"code_edit", "codebase_edit"}:
        raise ValueError(f"Unsupported fallback tool: {tool_name}")

    result = await handle_codebase_edit(arguments)
